...
3. 浏览器打开设备 IP 或 `http://d2m.local/`， 1. 给设备上电。
2. 如果设备没有连上已配置 Wi-Fi，会启动 AP，名称为 `d2m-<mac>`，默认密码为 `1234567890` 或无密码。
3. 浏览器打开设备 IP 或 `http://d2m.local/`。
...
以下说明中，\{macaddress\}为网关mac地址，需要从配置中查找(不是路由器上发现的mac地址)。\{adr\}为短地址，有效值为00-79 或 255，2位数字，0-9需要写成00-09，64-79表示组0到组15， 255表示全部（广播地址）。  
