...
5. 多功能键操作：
    ...
    - 长按（2秒以上，5秒以下,以松开时间为准，下同）：对新灯具分配短地址；
    - 长按（6秒以上）：删除所有已分配的地址；
...
2. 如果30秒内仍不能接入ap，则自动开启 ap 模式，其wifi ssid 以 d2m- 开头后跟 mac 地址，如 d2m-c049ef3f40b4，密码为1234567890；
3. ap 模式启动后，如果120秒内没有设备接入，则系统重启；如果有设备接入或尝试接入，则会一直停留在ap模式；
